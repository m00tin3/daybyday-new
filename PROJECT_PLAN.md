...
> 状态：**已定稿待开工**。v0.1 审查意见已全部落入本版：
> ① 项目名已定 ② 前端选 Vue3 ③ 秒杀保留 ④ 免费服务器部署 ⑤ 接口文档驱动联调 ⑥ 偏 Redis 实战但要能上线
...
### 1.2 现有资产
- `tieba-demo/index.html`：贴吧风格静态首页（作为视觉参考，前端将用 Vue3 重写）
...
| 页面 | 路由 | 说明 | 对应后端接口 |
|---|---|---|---|
| 首页 | `/` | 帖子列表、热门吧、热议话题 | 帖子列表、热吧榜 |
| 帖子详情 | `/post/:id` | 正文 + 楼层评论 + 点赞/收藏 | 帖子详情、评论列表 |
| 发帖 | `/post/new` | 编辑器 + 提交 | 发帖 |
| 登录/注册 | `/login` | 验证码登录、token 管理 | 登录接口 |
| 吧主页 | `/bar/:id` | 吧信息、帖子列表、签到入口 | 吧信息、签到 |
| 个人中心 | `/user/:id` | 我的帖子、收藏、签到日历、关注 | 用户信息、关注 |
| 搜索 | `/search` | 关键词搜索 + 热搜词展示 | 搜索、热搜 |
| 排行榜 | `/rank` | 热帖榜 / 热吧榜 | 榜单接口 |
| 同城 | `/nearby` | 附近帖子/线下活动（GEO） | GEO 接口 |
| 抢楼/徽章 | `/activity/:id` | 抢楼活动、限量徽章领取 | 秒杀接口 |
...
│   ├── views/          # 页面组件（对应上表）
│   ├── components/     # 公共组件（帖子卡片、评论楼层、点赞按钮等）
