...
| 16 | **楼中楼（固定 2 层）+ 回复提醒** | **子回复一次 `IN` 批量组装**（前 3 条预览 + 总数，不做 N+1）；`reply_to_user_id` 记录"你点的那条评论是谁的"，让通知发给**被回复的人**而不是楼主；通知把回复/点赞**统一封装**成一条消息（`type` 区分），**未读数走 DB COUNT 而非 Redis 计数器** |
