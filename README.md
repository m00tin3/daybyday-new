...
| 16 | **楼中楼（固定 2 层）+ 回复提醒** | **子回复一次 `IN` 批量组装**（每层带第 1 页 10 条 + 总数，不做 N+1）；超过 10 条**在该层内翻页**（仿贴吧），翻页按 `(post_id, parent_id)` 索引精确取一页；`reply_to_user_id` 记录"你点的那条评论是谁的"，让通知发给**被回复的人**而不是楼主；通知把回复/点赞**统一封装**成一条消息（`type` 区分），**未读数走 DB COUNT 而非 Redis 计数器** |
| 17 | **用户自主删除自己的帖子/回复** | **软删除**（`status=0`）但对用户伪装成真删除：前端与后台都**不提供任何恢复入口**，恢复只能管理员改库；**删楼层时其楼中楼保留可见**（占位 + 内容与作者脱敏）；**删帖只改帖子状态、不逐条改楼层** —— 回复者「TA 的回复」记录仍在、点击提示"此帖已被删除" |
...
6. **帖子详情**：点赞/收藏（Set）、回帖盖楼（楼层号 Redis INCR）、浏览/UV 计数
   - **楼中楼**：每层楼右侧「回复」→ 行内输入框 → 子回复缩进显示在该层下方，显示「回复 @某某」；只能两层（回复子回复时结构上仍挂在该顶层楼层下）
   - **消息**：顶栏「消息」带未读红点 → 进入通知页自动全部已读；被回复/被点赞都会收到，列表里能看出究竟是哪种
...
14. **删自己的内容**：个人主页「TA 的帖子」/「TA 的回复」两个 Tab 里，自己发的每条都带「删除」；浏览帖子时自己的帖子/楼层/子回复也都有「删除」。删楼层后该位置变成「该楼层已被删除」占位，**别人的楼中楼回复照常显示**；删帖后整帖消失，但回复过它的人主页里那条记录仍在，点一下会提示「此帖已被删除」
...
│   │                           # migration_notification.sql 存量库增量迁移（楼中楼 + 通知表）
