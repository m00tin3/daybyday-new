...
/**
 * 登录态拦截器（🔒 接口）。
 * <p>流程：取 {@code Authorization: Bearer <token>} → 查 Redis {@code dbd:login:token:<token>}
 * → 命中则写入 {@link UserContext} 并续期（滑动过期 30 分钟），未命中返回 HTTP 401。
 * token 与用户 ID 的绑定关系在登录接口写入（见 AuthService）。</p>
 */
